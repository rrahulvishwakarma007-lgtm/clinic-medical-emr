...
import { getMedicineHindi } from "@/lib/medicineHindi";
import { getLabTestHindi } from "@/lib/labTestHindi";

// Allow generatePDF on iframe contentWindow
declare global { interface Window { generatePDF?: () => void; } }
...
function PrescriptionsPageInner() {
  ...
  // ── Mobile print preview (in-app iframe instead of window.open) ──
  const [printPreviewHtml, setPrintPreviewHtml] = useState<string | null>(null);
  const [printPreviewData, setPrintPreviewData] = useState<any | null>(null);
  const [previewZoom, setPreviewZoom] = useState(1); // zoom level for prescription preview
  ...
  // ── Open print — in-app on mobile, new window on desktop ─────────────────
  function openPrint(htmlContent: string) {
    if (isMobileDevice()) {
      // Show in-app preview modal — back button closes it, not the app
      setPrintPreviewHtml(htmlContent);
    } else {
      // Desktop — open in new window as before
  ...
  function handlePrintLabReq(order: LabOrder) {
    ...
    const w = isMobile ? null : window.open("", "_blank", "width=800,height=700");
    ...
    function getLabPurpose(code: string, name: string): string {
      // 1. Check existing code-based map first
      if (LAB_PURPOSE_MAP[code]) return LAB_PURPOSE_MAP[code];
      // 2. Fall back to comprehensive name-based labTestHindi DB
      const fromDB = getLabTestHindi(name);
      if (fromDB) return fromDB;
      return "";
    }

    const labHtml = `<!DOCTYPE html><html lang="hi"><head><meta charset="utf-8"/><title>Lab Request - ${order.patient_name}</title>
    ...
    // For mobile — remove auto-print (PDF handled by parent Share button)
    const labHtmlMobile = labHtml.replace(
      `<script>window.onload=()=>{window.print();}<\\/script>`,
      `<script>/* PDF/Share handled by parent window */<\\/script>`
    );

    if (isMobileDevice()) {
      setPrintPreviewHtml(labHtmlMobile);
      setPrintPreviewData({ type: "lab", ...order });
    } else {
      const w2 = window.open("", "_blank", "width=800,height=700");
      if (w2) { w2.document.write(labHtml); w2.document.close(); }
    }
  }
  ...
  function handlePrint(p: any) {
    ...
    function hindiMedicinePurpose(medName: string): string {
      if (!medName) return "";
      const lower = medName.toLowerCase();
      // 1. Check existing keyword array first (most specific)
      for (const entry of MED_PURPOSE) {
        if (entry.keywords.some(kw => lower.includes(kw))) {
          return entry.purpose;
        }
      }
      // 2. Fall back to the comprehensive medicineHindi DB (exact + prefix match)
      const fromDB = getMedicineHindi(medName);
      if (fromDB) return fromDB;
      return "";
    }
    ...
    const rxHtml = `<!DOCTYPE html><html lang="hi"><head>
    <meta charset="utf-8"/>
    ...
    // For mobile — remove auto-print (PDF handled by parent Share button)
    const rxHtmlMobile = rxHtml.replace(
      `<script>window.onload=()=>window.print()<\\/script>`,
      `<script>/* PDF/Share handled by parent window */<\\/script>`
    );
    if (isMobileDevice()) {
      setPrintPreviewHtml(rxHtmlMobile);
      setPrintPreviewData({ type: "prescription", ...p, patientName });
    } else {
      const w = window.open("", "_blank", "width=860,height=1000");
      if (w) { w.document.write(rxHtml); w.document.close(); }
    }
  }
  ...
  return (
    <div style={{ padding: "2rem", minHeight: "100vh", background: "#f0f4f8", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* ── Mobile In-App Print Preview ── */}
      {printPreviewHtml && (
        <div style={{ position:"fixed", inset:0, zIndex:9999, background:"#1a1a2e", display:"flex", flexDirection:"column" }}>

          {/* ── Top bar — all buttons side by side as icons ── */}
          <div style={{
            display:"flex", alignItems:"center", gap:"6px",
            padding:"8px 10px", background:"#0f4c81", flexShrink:0, flexWrap:"nowrap",
          }}>

            {/* Back */}
            <button onClick={() => { setPrintPreviewHtml(null); setPrintPreviewData(null); setPreviewZoom(1); }}
              title="Back"
              style={{ background:"rgba(255,255,255,0.15)", border:"none", color:"white", borderRadius:"8px", padding:"8px 10px", fontSize:"16px", cursor:"pointer", flexShrink:0 }}>
              ←
            </button>

            {/* Title */}
            <span style={{ fontWeight:"700", fontSize:"12px", color:"white", flex:1, textAlign:"center", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
              {printPreviewData?.type === "lab" ? "🔬 Lab Request" : "💊 Prescription"}
            </span>

            {/* Zoom Out */}
            <button onClick={() => setPreviewZoom(z => Math.max(0.4, +(z - 0.15).toFixed(2)))}
              title="Zoom Out"
              style={{ background:"rgba(255,255,255,0.15)", border:"none", color:"white", borderRadius:"8px", padding:"8px 10px", fontSize:"15px", cursor:"pointer", flexShrink:0 }}>
              🔍−
            </button>
            ...
            <button onClick={() => setPreviewZoom(1)}
              title="Reset Zoom"
              style={{ background:"rgba(255,255,255,0.1)", border:"none", color:"rgba(255,255,255,0.8)", borderRadius:"8px", padding:"6px 8px", fontSize:"11px", cursor:"pointer", flexShrink:0, fontWeight:"700", minWidth:"38px" }}>
              {Math.round(previewZoom * 100)}%
            </button>

            {/* Zoom In */}
            <button onClick={() => setPreviewZoom(z => Math.min(2.5, +(z + 0.15).toFixed(2)))}
              title="Zoom In"
              style={{ background:"rgba(255,255,255,0.15)", border:"none", color:"white", borderRadius:"8px", padding:"8px 10px", fontSize:"15px", cursor:"pointer", flexShrink:0 }}>
              🔍+
            </button>
            ...
            <button
              ...
              onClick={() => {
                const d = printPreviewData;
                if (!d) return;
                let msg = "";
                if (d.type === "prescription") {
                  const meds = (d.medicine || "").split("\n");
                  const dosages = (d.dosage || "").split("\n");
                  const durations = (d.duration || "").split("\n");
                  const routes = (d.route || "").split("\n");
                  const date = new Date(d.created_at || Date.now()).toLocaleDateString("en-IN", { day:"2-digit", month:"short", year:"numeric" });
                  msg = `🏥 *${hospitalConfig.name}*\n👨‍⚕️ ${hospitalConfig.doctorName} (${hospitalConfig.doctorDegree})\n📅 ${date}\n─────────────────────\n👤 *Patient:* ${d.patientName || d.patients?.name || "—"}\n`;
                  if (d.diagnosis) msg += `🩺 *Diagnosis:* ${d.diagnosis}\n`;
                  msg += `─────────────────────\n💊 *Prescribed Medicines:*\n\n`;
                  meds.forEach((med: string, i: number) => {
                    if (!med.trim()) return;
                    msg += `*${i + 1}. ${med.trim()}*\n`;
                    if (dosages[i]?.trim()) msg += `   📋 Dose: ${dosages[i].trim()}\n`;
                    if (durations[i]?.trim()) msg += `   ⏱ Duration: ${durations[i].trim()}\n`;
                    if (routes[i]?.trim()) msg += `   💉 Route: ${routes[i].trim()}\n`;
                    msg += `\n`;
                  });
                  if (d.notes) msg += `📝 *Instructions:* ${d.notes}\n\n`;
                  if (d.followup_date) { const fu = new Date(d.followup_date).toLocaleDateString("en-IN", { day:"2-digit", month:"short", year:"numeric" }); msg += `📅 *Follow-up:* ${fu}\n\n`; }
                  msg += `─────────────────────\n_${hospitalConfig.name}_`;
                } else {
                  const date = new Date().toLocaleDateString("en-IN", { day:"2-digit", month:"short", year:"numeric" });
                  msg = `🏥 *${hospitalConfig.name}*\n👨‍⚕️ ${hospitalConfig.doctorName}\n📅 ${date}\n─────────────────────\n👤 *Patient:* ${d.patient_name || "—"}\n─────────────────────\n🔬 *Lab Tests:*\n\n`;
                  const statT = (d.tests||[]).filter((t:any)=>t.urgency==="STAT");
                  const urgentT = (d.tests||[]).filter((t:any)=>t.urgency==="Urgent");
                  const routineT = (d.tests||[]).filter((t:any)=>t.urgency==="Routine");
                  if (statT.length) { msg += `🔴 *STAT:*\n`; statT.forEach((t:any)=>{ msg+=`  • ${t.name}\n`; }); msg+=`\n`; }
                  if (urgentT.length) { msg += `🟡 *Urgent:*\n`; urgentT.forEach((t:any)=>{ msg+=`  • ${t.name}\n`; }); msg+=`\n`; }
                  if (routineT.length) { msg += `🟢 *Routine:*\n`; routineT.forEach((t:any)=>{ msg+=`  • ${t.name}\n`; }); msg+=`\n`; }
                  msg += `─────────────────────\n_${hospitalConfig.name}_`;
                }
                const phone = printPreviewData?.phone || patients.find((pt:any)=>pt.id===printPreviewData?.patient_id)?.phone || "";
                const cleanPhone = phone.replace(/\D/g,"");
                const waUrl = cleanPhone ? `https://wa.me/91${cleanPhone}?text=${encodeURIComponent(msg)}` : `https://wa.me/?text=${encodeURIComponent(msg)}`;
                window.open(waUrl, "_blank");
              }}
              style={{ background:"#25D366", border:"none", color:"white", borderRadius:"8px", padding:"8px 10px", fontSize:"16px", cursor:"pointer", flexShrink:0 }}>
            ...
            <button
              title="Print"
              onClick={() => { const f = document.getElementById("print-preview-iframe") as HTMLIFrameElement; if (f?.contentWindow) f.contentWindow.print(); }}
              style={{ background:"white", border:"none", color:"#0f4c81", borderRadius:"8px", padding:"8px 10px", fontSize:"16px", cursor:"pointer", flexShrink:0 }}>
              🖨
            </button>

            {/* PDF — save HTML to localStorage then open print page in same window */}
            <button
              title="Open in Browser for PDF"
              onClick={() => {
                try {
                  // Save HTML to localStorage so print page can read it
                  localStorage.setItem("emr_print_html", printPreviewHtml || "");
                  localStorage.setItem("emr_print_type", printPreviewData?.type || "prescription");
                  // Navigate to print page in same window — this opens in WebView
                  // but user can then use browser share menu
                  const printUrl = `${window.location.origin}/print-preview`;
                  // Use location.href — this works in Capacitor unlike window.open
                  window.location.href = printUrl;
                } catch(e) {
                  alert("Could not open browser. Please use the 🖨 Print button instead.");
                }
              }}
              style={{ background:"#dc2626", border:"none", color:"white", borderRadius:"8px", padding:"8px 10px", fontSize:"15px", cursor:"pointer", flexShrink:0, fontWeight:"700" }}>
              📄
            </button>

          </div>

          {/* Zoomable prescription — using transform:scale on iframe */}
          <div style={{ flex:1, overflow:"auto", background:"#1a1a2e", position:"relative" }}>
            <div style={{
              // This outer div is scrollable and sized to fit the scaled iframe
              minHeight: "100%",
              display: "flex",
              ...
              <div style={{
                // Transform origin top-center so zoom feels natural
                ...
                // Height must be set so parent scrollable div knows the size
                height: "1100px",
              }}>
                <iframe
          ...
          {/* Bottom zoom bar */}
          <div style={{ background:"#0a1628", padding:"10px 16px", display:"flex", alignItems:"center", justifyContent:"center", gap:"20px", flexShrink:0 }}>
            <button onClick={() => setPreviewZoom(z => Math.max(0.4, +(z - 0.15).toFixed(2)))}
              style={{ background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.2)", color:"white", borderRadius:"50%", width:"40px", height:"40px", fontSize:"20px", cursor:"pointer", fontWeight:"700", display:"flex", alignItems:"center", justifyContent:"center" }}>
              −
            </button>
            <div style={{ color:"rgba(255,255,255,0.7)", fontSize:"13px", textAlign:"center" }}>
              <div style={{ fontWeight:"700", fontSize:"16px", color:"white" }}>{Math.round(previewZoom * 100)}%</div>
