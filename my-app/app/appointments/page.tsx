...
export default function AppointmentsPage() {
  ...
  const [editForm, setEditForm] = useState({
    date: "", time: "", visit_type: "General Checkup", notes: "", status: "Confirmed" as Status,
  });
  ...
  const [form, setForm] = useState({
    patient_id: "", patient_name: "", date: today, time: "",
    visit_type: "General Checkup", notes: "", status: "Confirmed" as Status,
  });
  ...
  async function loadAppointments() {
    setPageLoading(true);
    try {
      const { data, fromCache: cached, cacheTime: ct } = await fetchWithCache(
        "/api/appointments",
  ...
  async function loadPatients() {
    try {
      const { data } = await fetchWithCache("/api/patients", CACHE_KEYS.patients);
    ...
    try {
      const res = await fetch("/api/appointments", {
        ...
        body: JSON.stringify({
          ...
          date: form.date, time: form.time, visit_type: form.visit_type,
  ...
  function openEdit(a: any) {
    ...
    setEditForm({ date: a.date, time: a.time, visit_type: a.visit_type || "General Checkup", notes: a.notes || "", status: a.status || "Confirmed" });
  }
  ...
  async function saveEdit() {
    ...
    try {
      const res = await fetch("/api/appointments", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: editAppointment.id, ...editForm }),
      });
      const data = await res.json();
      if (data?.error) { alert("Error: " + data.error); return; }
      setAppointments(prev => prev.map(a => a.id === editAppointment.id ? { ...a, ...editForm } : a));
  ...
  return (
    <div style={{ padding: "2rem", minHeight: "100vh", background: "#f0f4f8", fontFamily: "'DM Sans', sans-serif", boxSizing: "border-box", maxWidth: "100%" }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        @import url('https://fonts.googleapis.com/css2?family=DM+Serif+Display&family=DM+Sans:wght@300;400;500;600;700&display=swap');

        .view-btn { border:none; cursor:pointer; padding:8px 18px; border-radius:8px; font-size:13px; font-weight:500; transition:all 0.15s; font-family:inherit; white-space:nowrap; }
        ...
        .edit-btn { background:#f0fdf4; color:#166534; border:none; padding:5px 10px; border-radius:6px; font-size:11px; font-weight:700; cursor:pointer; transition:all 0.15s; font-family:inherit; white-space:nowrap; }
        .edit-btn:hover { background:#dcfce7; }

        .view-appt-btn { background:#ebf8ff; color:#3182ce; border:none; padding:5px 10px; border-radius:6px; font-size:11px; font-weight:700; cursor:pointer; font-family:inherit; white-space:nowrap; }
        .view-appt-btn:hover { background:#bee3f8; }
        .call-appt-btn { background:#f0fdf4; color:#15803d; border:1.5px solid #bbf7d0; padding:5px 10px; border-radius:6px; font-size:11px; font-weight:700; cursor:pointer; font-family:inherit; white-space:nowrap; text-decoration:none; display:inline-flex; align-items:center; gap:3px; transition:all 0.15s; }
        .call-appt-btn:hover { background:#dcfce7; border-color:#86efac; }
        ...
        /* ── APPOINTMENTS TABLE SCROLL WRAPPER ── */
        .appt-table-wrap {
          overflow-x: auto;
        ...
        @media (max-width: 768px) {
          .appt-page-header { flex-direction: column !important; align-items: stretch !important; gap: 10px !important; }
          .appt-new-btn { width: 100% !important; text-align: center !important; justify-content: center !important; }
          .appt-stats-grid { grid-template-columns: 1fr 1fr !important; gap: 10px !important; }
          .appt-filters { flex-direction: column !important; align-items: stretch !important; }
          .appt-filters input { width: 100% !important; }

          /* CRITICAL: force all cells visible — overrides globals.css hide rules */
          .appt-table-wrap table th,
          .appt-table-wrap table td {
          ...
          .appt-table-wrap table td:last-child {
            display: table-cell !important;
          }

          .appt-table-wrap table th:nth-child(4),
          .appt-table-wrap table td:nth-child(4) {
            display: table-cell !important;
          }

          /* Action buttons compact on mobile */
          .appt-table-wrap .del-btn { padding: 4px 6px !important; font-size: 12px !important; }
          .appt-table-wrap .edit-btn { padding: 4px 7px !important; font-size: 10px !important; }
          .appt-table-wrap .view-appt-btn { padding: 4px 7px !important; font-size: 10px !important; }
          .appt-table-wrap .call-appt-btn { padding: 4px 7px !important; font-size: 10px !important; }

          /* Stats card compact */
          .stat-card { padding: 12px 14px !important; }
          .stat-card div[style*="font-size: 26px"] { font-size: 22px !important; }
        }
      `}</style>

      {/* ── Header ── */}
      <div className="appt-page-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
        <div>
        ...
        <button className="appt-new-btn" onClick={() => setShowAdd(true)}
          disabled={isOffline}
          title={isOffline ? "Cannot add appointment while offline" : ""}
          style={{ background: isOffline ? "#e2e8f0" : "#0f4c81", color: isOffline ? "#94a3b8" : "white", border: "none", padding: "11px 22px", borderRadius: "10px", cursor: isOffline ? "not-allowed" : "pointer", fontWeight: "600", fontSize: "14px", boxShadow: isOffline ? "none" : "0 4px 14px rgba(15,76,129,0.25)", whiteSpace: "nowrap" }}>
          {isOffline ? "📵 Offline" : "+ New Appointment"}
        </button>
      </div>
      ...
      {fromCache && cacheTime && (
        <div style={{
          ...
          borderLeft: "4px solid #d97706", borderRadius: "10px",
          padding: "10px 16px", fontSize: "12px", color: "#92400e",
      ...
      {/* ── Stats ── */}
      <div className="appt-stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "14px", marginBottom: "22px" }}>
        {[
      ...
      {/* ── Filters ── */}
      <div className="appt-filters" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", gap: "12px", flexWrap: "wrap" }}>
        ...
        <input type="text" placeholder="Search patient..." value={searchQuery}
          ...
          style={{ padding: "9px 14px", borderRadius: "8px", border: "1.5px solid #e2e8f0", fontSize: "14px", width: "220px", background: "white", boxSizing: "border-box" }} />
      </div>

      {/* ── Table — wrapped in appt-table-wrap for horizontal scroll ── */}
            ...
            <thead>
              <tr style={{ background: "#0f4c81" }}>
                {["Time", "Patient", "Visit Type", "Date", "Notes", "Status", "Actions"].map(h => (
                  <th key={h} style={{ padding: "13px 16px", color: "white", fontSize: "11px", textTransform: "uppercase", letterSpacing: "1.5px", textAlign: "left", fontWeight: "600", whiteSpace: "nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {pageLoading ? (
                ...
                return (
                  <tr key={a.id} className="appt-row" style={{ borderBottom: "1px solid #f0f0f0", background: i % 2 === 0 ? "#fff" : "#fafbfc" }}>
                    <td style={{ padding: "13px 16px", fontWeight: "700", color: "#0f4c81", fontSize: "14px", whiteSpace: "nowrap" }}>{formatTime(a.time)}</td>
                    <td style={{ padding: "13px 16px", fontWeight: "600", color: "#1a1a2e", fontSize: "14px", whiteSpace: "nowrap" }}>{a.patient_name}</td>
                    <td style={{ padding: "13px 16px", fontSize: "13px", color: "#555", whiteSpace: "nowrap" }}>{a.visit_type || "General Checkup"}</td>
                    <td style={{ padding: "13px 16px", fontSize: "13px", color: "#666", whiteSpace: "nowrap" }}>{formatDate(a.date)}</td>
                    <td style={{ padding: "13px 16px", fontSize: "12px", color: "#999", maxWidth: "140px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.notes || "—"}</td>
                    <td style={{ padding: "13px 16px", whiteSpace: "nowrap" }}>
                      <select className="status-select" value={a.status || "Confirmed"}
                        disabled={updatingId === a.id || isOffline}
                        style={{ background: st.bg, color: st.color, opacity: isOffline ? 0.6 : 1 }}
                    ...
                    <td style={{ padding: "13px 16px", whiteSpace: "nowrap" }}>
                      <div style={{ display: "flex", gap: "5px", alignItems: "center" }}>
                        {(() => {
                          const phone = patients.find(p => String(p.id) === String(a.patient_id))?.phone || a.phone;
                          return phone ? <CallButton phone={phone} patientName={a.patient_name} size="sm" /> : null;
                        })()}
                        <button className="view-appt-btn" onClick={() => setViewAppointment(a)}>👁 View</button>
      ...
      {showAdd && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(10,20,40,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
          <div className="modal-anim" style={{ background: "white", borderRadius: "16px", width: "520px", maxWidth: "100%", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 60px rgba(0,0,0,0.25)" }}>
            ...
            <div style={{ padding: "0 32px 28px", display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              <div>
                ...
                <textarea placeholder="Reason for visit, symptoms, special instructions..."
          ...
          <div className="modal-anim" style={{ background: "white", borderRadius: "16px", width: "520px", maxWidth: "100%", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 60px rgba(0,0,0,0.25)" }}>
            ...
            <div style={{ padding: "0 32px 28px", display: "flex", flexDirection: "column", gap: "16px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "8px" }}>
                  {TIME_SLOTS.map(t => {
                    const taken = appointments.some(a =>
                      a.date === editForm.date && a.time === t &&
                      a.status !== "Cancelled" && a.id !== editAppointment.id
                    );
                    return (
              ...
              <div>
                ...
                <textarea placeholder="Reason for visit, symptoms..."
                  style={{ ...inputStyle, minHeight: "80px", resize: "none" }}
      ...
      {viewAppointment && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(10,20,40,0.65)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100, padding: "20px" }}>
          <div className="modal-anim" style={{ background: "white", borderRadius: "16px", width: "500px", maxWidth: "100%", padding: "32px", boxShadow: "0 24px 60px rgba(0,0,0,0.3)", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
              ...
              <div style={{ marginTop: "4px", display: "flex", gap: "10px", flexWrap: "wrap" }}>
                <button onClick={() => setViewAppointment(null)}
                  style={{ flex: 1, minWidth: "80px", padding: "12px", borderRadius: "10px", border: "1.5px solid #e2e8f0", background: "white", cursor: "pointer", fontWeight: "700", fontSize: "14px", color: "#4a5568" }}>
                  Close
                </button>
                {(() => {
                  const phone = patients.find(p => String(p.id) === String(viewAppointment.patient_id))?.phone || viewAppointment.phone;
                  return phone ? (
                    <div style={{ flex: 1, minWidth: "80px" }}>
                      <CallButton phone={phone} patientName={viewAppointment.patient_name} size="md" />
                    </div>
                  ) : null;
                })()}
                <button onClick={() => { setViewAppointment(null); openEdit(viewAppointment); }}
                  style={{ flex: 1, minWidth: "80px", padding: "12px", borderRadius: "10px", border: "none", background: "#f0fdf4", color: "#166534", cursor: "pointer", fontWeight: "700", fontSize: "14px" }}>
                  ✏️ Edit
                </button>
                <button onClick={() => { const pid = viewAppointment.patient_id; setViewAppointment(null); router.push(`/patients/${pid}`); }}
                  style={{ flex: 1, minWidth: "80px", padding: "12px", borderRadius: "10px", border: "none", background: "#0f4c81", color: "white", cursor: "pointer", fontWeight: "700", fontSize: "14px" }}>
                  View Profile
                </button>
              </div>
            </div>
