...
export default function ReportsPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      // try loading uploads separately
      try {
  ...
  const bloodGroups = patients.reduce((acc: any, p) => {
    if (p.blood_group) acc[p.blood_group] = (acc[p.blood_group] || 0) + 1;
    return acc;
  }, {});

  const serviceRevenue = billing.reduce((acc: any, b) => {
    acc[b.service_name] = (acc[b.service_name] || 0) + Number(b.amount);
    return acc;
  }, {});

  const topMedicines = prescriptions.reduce((acc: any, p) => {
  ...
  function printPatients() {
    ...
    const summary = `<div class="summary"><div class="stat"><div class="stat-val">${patients.length}</div><div class="stat-label">Total Patients</div></div></div>`;
    printReport("Patient Register", `<table><thead><tr><th>Name</th><th>Age</th><th>Blood Group</th><th>Phone</th><th>Type</th><th>Registered</th></tr></thead><tbody>${rows}</tbody></table>`, summary);
  }
  ...
  function printBilling() {
    ...
    const summary = `<div class="summary"><div class="stat"><div class="stat-val">${billing.length}</div><div class="stat-label">Total Invoices</div></div><div class="stat"><div class="stat-val">Rs.${totalRevenue.toLocaleString("en-IN")}</div><div class="stat-label">Collected</div></div><div class="stat"><div class="stat-val">Rs.${pendingRevenue.toLocaleString("en-IN")}</div><div class="stat-label">Pending</div></div></div>`;
    printReport("Billing & Revenue Report", `<table><thead><tr><th>Patient</th><th>Service</th><th>Amount</th><th>Date</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table>`, summary);
  }
  ...
  function printAppointments() {
    ...
    const summary = `<div class="summary"><div class="stat"><div class="stat-val">${appointments.length}</div><div class="stat-label">Total</div></div><div class="stat"><div class="stat-val">${todayAppts.length}</div><div class="stat-label">Today</div></div></div>`;
    printReport("Appointments Report", `<table><thead><tr><th>Patient</th><th>Date</th><th>Time</th><th>Type</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table>`, summary);
  }
  ...
  function printPrescriptions() {
    ...
    const summary = `<div class="summary"><div class="stat"><div class="stat-val">${prescriptions.length}</div><div class="stat-label">Total Rx</div></div></div>`;
    printReport("Prescription Report", `<table><thead><tr><th>Patient</th><th>Medicine</th><th>Dosage</th><th>Duration</th><th>Date</th></tr></thead><tbody>${rows}</tbody></table>`, summary);
  }
  ...
  async function handleDownload(report: any) {
    try {
      ...
      const data = await res.json();
      if (data.url) { const a = document.createElement("a"); a.href = data.url; a.download = report.name; a.click(); }
  ...
  const inputStyle: any = { width: "100%", padding: "10px 14px", borderRadius: "8px", border: "1.5px solid #e2e8f0", fontSize: "14px", boxSizing: "border-box" };

  return (
    <div style={{ padding: "2rem", minHeight: "100vh", background: "#f0f4f8", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Serif+Display&family=DM+Sans:wght@300;400;500;600;700&display=swap');
        .tab-btn{border:none;cursor:pointer;padding:10px 18px;border-radius:8px;font-size:14px;font-weight:500;transition:all 0.15s;display:flex;align-items:center;gap:6px}
        ...
        .print-btn{background:#0f4c81;color:white;border:none;padding:9px 20px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:600;display:flex;align-items:center;gap:6px;transition:all 0.15s}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px", flexWrap: "wrap", gap: "12px" }}>
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
          ...
          {(dateFrom || dateTo) && (
            <button onClick={() => { setDateFrom(""); setDateTo(""); }} style={{ padding: "8px 12px", borderRadius: "8px", border: "1px solid #ddd", background: "white", cursor: "pointer", fontSize: "12px", color: "#888" }}>
              Clear
            </button>
          )}
        </div>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: "80px", color: "#aaa" }}>Loading reports...</div>
      ) : (
        <>
          {/* OVERVIEW */}
          {activeTab === "overview" && (
                    ...
                    Object.entries(bloodGroups).map(([bg, count]: any) => (
                      <div key={bg} style={{ marginBottom: "12px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", marginBottom: "4px" }}>
                          ...
                          <span style={{ color: "#888" }}>{count} patients</span>
                        </div>
                    ...
                    Object.entries(topMedicines).sort((a: any, b: any) => b[1] - a[1]).slice(0, 6).map(([med, count]: any) => (
                      <div key={med} style={{ marginBottom: "12px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", marginBottom: "4px" }}>
                          ...
                          <span style={{ color: "#888" }}>{count}x prescribed</span>
                        </div>
                        <div style={{ background: "#f0f4f8", borderRadius: "4px", height: "8px" }}>
            ...
            <div>
              ...
              <div style={{ background: "white", borderRadius: "14px", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                <table className="data-table">
                  ...
                  <tbody>
                    {filterByDate(patients).length === 0 ? (
                      <tr><td colSpan={8} style={{ textAlign: "center", padding: "40px", color: "#bbb" }}>No patients found</td></tr>
            ...
            <div>
              ...
              <div style={{ background: "white", borderRadius: "14px", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                <table className="data-table">
                  ...
                  <tbody>
                    {filterByDate(billing).length === 0 ? (
                      <tr><td colSpan={8} style={{ textAlign: "center", padding: "40px", color: "#bbb" }}>No billing records</td></tr>
          ...
          {activeTab === "appointments" && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                <div style={{ fontWeight: "700", color: "#1a1a2e", fontSize: "16px" }}>Appointments Report <span style={{ color: "#888", fontWeight: "400", fontSize: "13px" }}>({filterByDate(appointments).length} records)</span></div>
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {filterByDate(appointments).length === 0 ? (
                      <tr><td colSpan={6} style={{ textAlign: "center", padding: "40px", color: "#bbb" }}>No appointments found</td></tr>
          ...
          {activeTab === "prescriptions" && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                <div style={{ fontWeight: "700", color: "#1a1a2e", fontSize: "16px" }}>Prescription Report <span style={{ color: "#888", fontWeight: "400", fontSize: "13px" }}>({filterByDate(prescriptions).length} records)</span></div>
              ...
              <div style={{ background: "white", borderRadius: "14px", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                <table className="data-table">
                  ...
                  <tbody>
                    {filterByDate(prescriptions).length === 0 ? (
                      <tr><td colSpan={7} style={{ textAlign: "center", padding: "40px", color: "#bbb" }}>No prescriptions found</td></tr>
          ...
          {activeTab === "uploads" && (
            <div style={{ display: "grid", gridTemplateColumns: "360px 1fr", gap: "20px", alignItems: "start" }}>
              <div style={{ background: "white", borderRadius: "14px", padding: "24px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                ...
                <div style={{ fontSize: "12px", color: "#999", marginBottom: "18px" }}>Lab results, X-rays, scanned documents from outside labs</div>
                <div
                ...
                >
                  <input ref={fileRef} type="file" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx,.xls,.xlsx" style={{ display: "none" }} onChange={e => { const f = e.target.files?.[0]; if (f) setUploadForm(p => ({ ...p, file: f })); }} />
                  {uploadForm.file ? (
                    <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginTop: "14px" }}>
                  ...
                  <button onClick={handleUpload} disabled={uploading} style={{ padding: "11px", borderRadius: "8px", background: uploading ? "#93c5fd" : "#0f4c81", color: "white", border: "none", cursor: uploading ? "not-allowed" : "pointer", fontWeight: "600", fontSize: "14px" }}>
